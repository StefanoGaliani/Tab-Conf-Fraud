...
""" class ConfidenceHead(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, min_conf=0.05, max_conf=0.95):
        super().__init__()
    ...
    def forward(self, x):
        raw_conf = self.net(x)  # ∈ [0,1]
        # Rescale to [min_conf, max_conf] smoothly
        confidence = self.min_conf + (self.max_conf - self.min_conf) * raw_conf
        return confidence """

class ConfidenceHead(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, min_conf=0.05, max_conf=0.95, temperature=1):
        super().__init__()
        self.min_conf = min_conf
        self.max_conf = max_conf
        ...
        nn.init.constant_(self.net[2].bias, -0.5)  # Encourage low initial confidence

    def forward(self, x):
        raw_conf = self.net(x)  # Unbounded output

        # Apply temperature-scaled sigmoid
        scaled = torch.sigmoid(raw_conf / self.temperature)  # ∈ [0, 1], smoother

        # Rescale to [min_conf, max_conf]
        confidence = self.min_conf + (self.max_conf - self.min_conf) * scaled
        return confidence
...
class TabTransformer(Module):
    def __init__(
        ...
        *,
        ...
        self.conf_head = ConfidenceHead(input_size)
    ...
    def forward(self, x_categ, x_cont, return_attn = False):
        ...
        #print('TabTransformer Model Instance created')
        return (logits, confidence), attns
